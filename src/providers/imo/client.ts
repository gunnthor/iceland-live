...
/*
 * `envOr` rather than `??` throughout this block: a key that exists and is
 * blank is what a hosting dashboard produces when somebody adds it without
 * filling it in, and every one of these values would be actively harmful
 * empty. A blank base URL is not a relative base, it is an invalid one, and a
 * blank version pin is a header IMO answers with HTTP 400. See `lib/env.ts`.
 */
export const IMO_BASE_URL = envOr(process.env.IMO_API_BASE_URL, "https://api.vedur.is");
...
export const IMO_API_VERSIONS = {
  quakes: envOr(process.env.IMO_QUAKES_API_VERSION, "2026-08-06"),
  volcanoes: envOr(process.env.IMO_VOLCANOES_API_VERSION, "2026-06-04"),
  cap: envOr(process.env.IMO_CAP_API_VERSION, "2026-04-14"),
  epos: envOr(process.env.IMO_EPOS_API_VERSION, "2026-02-05"),
  /*
   ...
   */
  dispersion: envOr(process.env.IMO_DISPERSION_API_VERSION, "2025-08-13"),
} as const;
